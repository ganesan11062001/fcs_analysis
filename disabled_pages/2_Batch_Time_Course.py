...
with st.container(border=True):
    ...
    if st.button("Run batch", type="primary"):
        ...
        for i, f in enumerate(uploaded_files):
            ...
            progress.progress(i / len(uploaded_files), text=f"Processing {f.name} ({i + 1}/{len(uploaded_files)})...")
            ...
            progress.progress((i + 1) / len(uploaded_files), text=f"Processed {f.name} ({i + 1}/{len(uploaded_files)})")
...
with st.container(border=True):
    ...
    if "tauD_ch1" in comp_df or "tauD_ch2" in comp_df:
        ...
        fig = px.scatter(
            melted, x="label", y="tauD", color="channel", title="Diffusion time (tau_D) vs. time course",
            color_discrete_map={"tauD_ch1": SERIES_COLORS["acf_ch1"], "tauD_ch2": SERIES_COLORS["acf_ch2"]},
        )
    ...
    if "bound_fraction_ch2_species" in comp_df:
        ...
        plot_cols[1].plotly_chart(apply_chart_style(fig2), width="stretch")
