...
info_cols[2].metric("Raw dt", f"{trace.dt * 1e6:.1f} us")
...
st.caption(f"Effective sampling rate after binning: **{effective_rate:,.1f} Hz** (dt = {effective_dt * 1e6:.2f} us)")
...
for kind, label in fit_panels:
    ...
    with fit_cols[col_idx]:
        ...
        if kind in fit_results:
            ...
            st.plotly_chart(plot_fit_overlay(results[kind].tau, results[kind].g, fr.fit_curve, title=label), width="stretch")
            st.write({"tauD": fr.tauD, "N": fr.N, "redchi": fr.redchi})
